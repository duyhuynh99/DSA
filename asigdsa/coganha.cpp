...
bool win(int** board, Move m, int player);
...
const int RANDOM_RATE = 8;

bool random = false;
int rate = RANDOM_RATE;

Move select_move(int **current_board, int **previous_board, int player)
{
    vector<Move> valid_move = get_valid_moves(current_board, previous_board, player);

    if (random == false) rate--;

    srand(time(NULL));
    int loto = rand() % rate;

    if (loto == 0) {
        int index = rand() % valid_move.size();

        random = true;
        rate = RANDOM_RATE;

        return valid_move.at(index);
    }
    ...
    int best_move = 0;
    int best_point = -10000;

    for (unsigned int i = 0; i < valid_move.size(); i++){
        point.push_back(get_score(current_board, valid_move.at(i), player));
        if (win(current_board, valid_move.at(i), player)) return valid_move.at(i);
    }

    for (unsigned int i = 0; i < valid_move.size(); i++){
        int** temp_current = copy_board(current_board);
        int** temp_previous = copy_board(previous_board);
        act_move(temp_current, temp_previous, valid_move.at(i), player);

        vector<Move> valid_move_oracle = get_valid_moves(temp_current, temp_previous, enemy);
        vector<int> point_oracle;

        for (unsigned int j = 0; j < valid_move_oracle.size(); j++)
            point_oracle.push_back(get_score(temp_current, valid_move_oracle.at(j), enemy));
        
        for (unsigned int j = 0; j < valid_move_oracle.size(); j++) 
        {
            int total_point = point.at(i) - point_oracle.at(j) + get_max_point_after(temp_current, valid_move_oracle.at(j), enemy);
            if (total_point > best_point) {
                best_point = total_point;
                best_move = i;
            }
        }
        destroy_marked(temp_current);
        destroy_marked(temp_previous);
    }
